<script>
  ...
  import {
    emotionColor, ratingToLabel, DYAD_BY_ID, SCORE_ALIASES,
    SENTIMENT_SCALE, sentimentToColor, metricScoreLabel, buildStageColorMap
  ...
  let {
    ...
    metrics = [],
    wheelData = null,
    illustrationSrc = '/static/illustrations/Regression1.jpeg',
    onopenEmotionDetail = undefined,
  } = $props();

  /** Controls the Experience Wheel sub-drawer */
  let wheelDrawerOpen = $state(false);
  ...
  // ── Tooltip state ──────────────────────────────────────────────────────────
  let hoveredMetricKey = $state(null);
  ...
  let step           = $derived($selectedIndex >= 0 ? data[$selectedIndex] : null);
  let sentimentLabel = $derived(step ? ratingToLabel(step.sentiment) : '');
  let stageColorMap  = $derived(buildStageColorMap(data));
  let stageColor     = $derived(
    (step && stageColorMap[step.stage_id]) ? stageColorMap[step.stage_id] : 'var(--jm-icon-dark, #3a3a3a)'
  );

  let tooltipMetric = $derived(hoveredMetricKey ? metrics.find(m => m.key === hoveredMetricKey) : null);
  let tooltipText   = $derived(
    (hoveredMetricKey && step?.metric_explainers)
      ? (step.metric_explainers[hoveredMetricKey] ?? null)
      : null
  );

  let emotionSwatches = $derived((() => {
    if (!step) return [];
  ...
  $effect(() => { if (step) sentimentTween.set(parseFloat(step.sentiment ?? 0)); });
  let sentimentColor = $derived(sentimentToColor($sentimentTween));

  // ── Tweened metric stores ──────────────────────────────────────────────────
  let metricTweens = [];
  let metricVals   = $state([]);
  ...
  $effect(() => { if (illustrationSrc) imgError = false; });
</script>
  ...
  <div class="content-wrap">
    ...
    <div class="toolbar p-2" style="border-bottom: 2.5px solid {stageColor}">
      <div class="flex flex-row gap-2 align-center items-center">
        ...
        <span class="pill-sm" style="background-color:{sentimentColor}; color:var(--ink)">
          {sentimentLabel}
        </span>
      </div>
...
<style>
  ...
  @media (max-width: 480px) {
    ...
    .content-padding { grid-template-columns: 1fr; }
  }
  ...
  .imb-squares--interactive { cursor: help; }
</style>
