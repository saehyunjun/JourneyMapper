<script>
  ...
  function ensureMeasureCtx() {
    ...
    measureCtx.font =
      '500 10px system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif';
  }
  ...
  const labelRowHeight = $derived(
    Math.max(1, ...wrappedSteps.map((lines) => lines.length)) * LINE_HEIGHT +
      LABEL_PAD_Y * 2
  );

  const svgHeight = $derived(labelRowHeight);

  function getStageWidth(group) {
