...
<script lang="ts">
  ...
  // Track previous filter values so we only rebuild when they actually change
  // — NOT on the initial $effect run, which would orphan dndzone's listeners.
  let prevSearch: string = searchQuery;
  let prevFilter: string | null = activeFilter;

  $effect(() => {
    const curSearch = searchQuery;
    const curFilter = activeFilter;

    if (curSearch === prevSearch && curFilter === prevFilter) return;
