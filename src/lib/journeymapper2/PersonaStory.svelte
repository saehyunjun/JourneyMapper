<script>
  ...
  $: SCREENS = [
    ...
    // 10. Paths
    inflectionStep?.inflection_detail?.paths?.length
      ? { id: 'paths', label: 'Pathways' }
      : null,
...
{#if open && persona}
  ...
  >
    ...
    <!-- ══ SCREEN: PATHS ══════════════════════════════════════════════════════
         Positive and negative path cards
    ═══════════════════════════════════════════════════════════════════════ -->
    {:else if screen === 'paths' && inflectionStep?.inflection_detail?.paths}
      {@const paths = inflectionStep.inflection_detail.paths}
      <div class="story-screen">
        <div class="content-wrap">
          <div class="jm-kicker flex items-center gap-2">
            <IconPathRegular /> Pathways
          </div>
          <div class="jm-section-bar">
            <span class="label-lg">How this moment could unfold</span>
          </div>

          <div class="flex flex-col gap-4">
            {#each paths as path}
              {@const isPos = path.direction === 'positive'}
              <div class="path-card path-card--{path.direction}">
                <div class="path-card-header">
                  ...
                  <span class="label-uppercase-bold" style="font-size:0.6rem">
                    {isPos ? 'Positive Path' : 'Negative Path'}
                  </span>
                  {#if path.sentiment_delta != null}
                    <span class="path-delta" class:path-delta--pos={isPos} class:path-delta--neg={!isPos}>
                ...
                {#if path.key_driver}
                  <div class="path-driver">
                    <span class="label-heading" style="font-size:0.55rem;text-transform:uppercase;letter-spacing:0.07em;opacity:0.65">Key driver</span>
                    <p class="text-body-sm" style="margin:0;font-size:0.72rem">{path.key_driver}</p>
                  </div>
                {/if}
              </div>
            {/each}
