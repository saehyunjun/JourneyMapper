...
<div class="jm-content-col gap-2 p-4">
  ...
  <div class="jm-content-col gap-3">
    <div class="jm-section-bar">
      <span class="label-sm uppercase">The Eight Primary Emotions</span>
    </div>
    ...
    <div class="emotions-grid">
      {#each primaryEmotions as e}
        <div class="card-body">
          ...
          <!-- Name + opposite -->
          <div class="toolbar-sm-empty py-1">
            <span class="label uppercase">{e.name}</span>
            <span class="jm-swatch-round-sm" style="background: {e.color};"></span>
          </div>
          ...
          <div class="flex flex-row w-full py-2">
            {#each e.intensity as level, idx}
              <span
                class="label-sm p-2 w-full"
                style="opacity: {0.4 + idx * 0.3}; background: {e.color}; color: var(--ink);"
              >
              {level}</span>
            {/each}
          </div>
          ...
          {#if e.journeyExamples.length}
            <div class="flex flex-col pt-4">
              <span class="label-xs">In this journey:</span>
              <div class="flex flex-row w-full gap-1 py-2">
                {#each e.journeyExamples as ex}
                  <span class="pill-sm" style="background: {e.color}; color: var(--ink);">{ex}</span>
                {/each}
              </div>
            </div>
          {/if}
  ...
  <!-- ── Dyads ─────────────────────────────────────────────────────────── -->
  <div class="jm-content-col gap-2">
      <span class="label-sm uppercase">Dyads Used in This Journey</span>
    </div>
    <p class="text-body-sm">Dyads are blended emotions formed when two adjacent primary emotions combine.</p>

    <div class="jm-content-col gap-2">
      {#each dyads as d}
        <div class="flex flex-row gap-2">
          <span class="jm-swatch" style="background: {d.color};"></span>
          <div class="flex flex-col gap-1">
            <div class="flex flex-row w-full gap-2">
              <span class="label uppercase">{d.components[0]} + {d.components[1]}</span>
              <IconArrowRightBold />
  ...
  <div class="flex flex-col pt-8 px-2">
    <div class="jm-section-bar">
      <span class="label-xs uppercase">Scoring in This Map</span>
    </div>
