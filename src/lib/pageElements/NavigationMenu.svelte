...
<NavigationMenu.Root class="relative z-10 flex w-full justify-center">
  ...
  >
    <NavigationMenu.Item value="getting-started">
      ...
      >
        Getting started
        <CaretDown
          class="relative  ml-1 size-3 transition-transform duration-200 group-data-[state=open]:rotate-180"
          aria-hidden="true"
        />
      </NavigationMenu.Trigger>
    ...
    >
      <div
        class="bg-border relative top-[70%] size-2.5 rounded-tl-[2px]"
      ></div>
    </NavigationMenu.Indicator>
  </NavigationMenu.List>
  <div
  ...
  >
    <NavigationMenu.Viewport
      class="text-popover-foreground bg-background data-[state=closed]:animate-scale-out data-[state=open]:animate-scale-in relative mt-2.5 w-full origin-[top_center] overflow-hidden rounded-md border shadow-lg duration-200  "
    />
  </div>
</NavigationMenu.Root>