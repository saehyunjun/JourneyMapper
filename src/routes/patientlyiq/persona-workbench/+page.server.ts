...
import { hasPersonaNarrative, listPersonas } from '$lib/server/personas';
...
export const load: PageServerLoad = async () => {
	...
	for (const c of corpora) {
		...
		corpora_authors[c.manifest.id] = [...byAuthor.entries()]
			.map(([author_handle_hash, slot]): AuthorRow => {
				...
				return {
					author_handle_hash,
					avatar_url: corpusParticipantAvatarUrl(c.manifest.id, author_handle_hash),
		...
		const raw = await loadCorpusArtifact<{
			...
			>;
			unattached_authors: string[];
		}>(c.manifest.id, 'persona_suggestions');
		if (raw) {
	...
	// Enrich each saved persona with whether a narrative sidecar already
	// exists. Drives the "Create narrative chart" vs. "View narrative"
	// affordance on the Personas tab without an extra round trip.
	//
	// Also enumerate which corpora already have a journey-map artifact for
	// this persona, so the UI can flag "Regenerate" vs. "Create journey-map
	// table" per the active corpus context.
	const allCorpusIds = corpora_summary.map((c) => c.id);
	const artifactNamesByCorpus: Record<string, Set<string>> = {};
	for (const cid of allCorpusIds) {
		artifactNamesByCorpus[cid] = new Set(await listCorpusArtifacts(cid));
	}
	const existing_personas = listPersonas().map((p) => {
		const has_narrative = hasPersonaNarrative(p.id);
		const journey_map_corpora = allCorpusIds.filter((cid) =>
			artifactNamesByCorpus[cid].has(`journey-map-${p.id}`)
		);
		return {
			...p,
			...(has_narrative ? { avatar_url: generatedPersonaAvatarUrl(p) } : {}),
			has_narrative,
			journey_map_corpora
		};
	});
